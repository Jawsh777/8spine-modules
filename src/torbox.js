...
const CONFIG = {
    MAX_SEARCH_LIMIT: 50,                    // Maximum number of search results to return
    TORRENT_POLL_ATTEMPTS: 20,               // TorBox polling attempts (20 × 2s = 40s timeout)
    REALDEBRID_POLL_ATTEMPTS: 30,            // Real-Debrid polling attempts (30 × 2s = 60s timeout, slower processing)
    POLL_INTERVAL: 2000,                     // Polling interval in milliseconds (2 seconds)
    MAX_QUERY_LENGTH: 500,                   // Maximum search query length
    RATE_LIMIT_MAX_RETRIES: 5,               // Maximum retries for rate-limited requests
    RATE_LIMIT_BASE_DELAY: 1000,             // Base delay for exponential backoff (1 second)
    RATE_LIMIT_MAX_DELAY: 30000,             // Maximum delay cap for rate limiting (30 seconds)
    CACHE_TTL: 3600 * 1000,                  // Cache entry lifetime (1 hour)
    CACHE_CLEANUP_INTERVAL: 600 * 1000,      // Cache cleanup interval (10 minutes)
    HASH_LENGTH_V1: 40,                      // BitTorrent v1 info hash length (SHA-1)
    HASH_LENGTH_V2: 32                       // BitTorrent v2 info hash length (Base32)
};
...
const ERRORS = {
    ...
    UNSUPPORTED_PROVIDER: (provider) => `Provider "${provider}" not supported for this feature. TorBox required.`
};

const AUDIO_EXTENSIONS = /\.(mp3|flac|wav|aac|ogg|wma|opus|ape|alac)$/i; // Removed m4a - it's too ambiguous with mp4
const VIDEO_PATTERNS = [
    ...
    /\.(mkv|mp4|avi|mov|wmv|m4v)$/i, // Video file extensions
];
...
//
// Cache Lifecycle:
// - Cache is initialized on module load and cleaned automatically every 10 minutes
// - Cache entries expire after 1 hour (CONFIG.CACHE_TTL)
// - The cleanup timer runs indefinitely for the lifetime of the module
// - stopCacheCleanup() is available but not called automatically
// - In environments that support module hot-reloading, cleanup should be called
//   manually before reload to prevent timer accumulation
//
// ============================================================================
...
function initCacheCleanup() {
    if (!cacheCleanupTimer) {
        cacheCleanupTimer = setInterval(cleanCache, CONFIG.CACHE_CLEANUP_INTERVAL);
    }
}
...
function validateHash(hash) {
    ...
    if (len !== CONFIG.HASH_LENGTH_V1 && len !== CONFIG.HASH_LENGTH_V2) {
        return false;
    }

    // V1 (SHA-1): 40 hex characters (case insensitive)
    if (len === CONFIG.HASH_LENGTH_V1) {
        return /^[a-fA-F0-9]{40}$/i.test(hash);
    }

    // V2 (Base32): 32 characters using A-Z and 2-7 (case insensitive)
    if (len === CONFIG.HASH_LENGTH_V2) {
...
function extractHash(magnet) {
    ...
    if (hash && !validateHash(hash)) {
        console.warn('[MusicTorrent] Invalid hash format extracted:', hash);
        return null;
    }
...
function normalizeMagnetPrefix(magnetLink) {
    if (!magnetLink) return magnetLink;

    // Handle double magnet: prefix - remove the first one
    if (magnetLink.startsWith('magnet:magnet:')) {
        return magnetLink.substring(7); // Remove first "magnet:"
    }
...
function parseTorrentName(name) {
    ...
    const qualityMatch = name.match(/\b(FLAC|MP3|320|256|V0|24bit|16bit|Hi-?Res|Lossless|WEB|CD|Vinyl)\b/i);
...
function getEffectiveDebridKey(context) {
    // Check module-level TorBox key first
    ...
    // Fall back to global debrid connection
    if (context?.debridApiKey) {
        return {
            apiKey: context.debridApiKey,
            provider: context.debridProvider,
            source: 'global'
        };
    }
...
async function getMagnetLink(infoHash, existingMagnet) {
    ...
    // Fallback: construct basic magnet with trackers
    return `magnet:?xt=urn:btih:${infoHash}&tr=` +
           TRACKERS.map(encodeURIComponent).join('&tr=');
}
...
async function checkCached(hashes, context) {
    ...
    try {
        ...
        const url = TORBOX_API_BASE + '/torrents/checkcached?hash=' +
                    validHashes.join(',') + '&format=object';
        ...
        // Convert to { hash: boolean } format
        const cacheStatus = {};
        const cachedData = data.data || {};

        // Normalize all cache data keys to lowercase for consistent lookup
        ...
        console.log('[MusicTorrent] Cache check result:', cachedCount, 'of',
                   validHashes.length, 'cached');
...
async function checkUsenetCached(hashes, context) {
    const debrid = getEffectiveDebridKey(context);
    ...
    // Filter valid hashes for usenet
    ...
    try {
        ...
        const url = TORBOX_API_BASE + '/usenet/checkcached?hash=' +
                    validHashes.join(',') + '&format=object';
        ...
        const cachedCount = Object.values(cacheStatus).filter(Boolean).length;
        console.log('[MusicTorrent] Usenet cache check result:', cachedCount, 'of',
                   validHashes.length, 'cached');
...
async function searchTracks(query, limit, context) {
    ...
    const searchSource = context?.settings?.searchSource?.value || 'torrents';

    // Route to appropriate search function based on source
    ...
    // Legacy 'usenet' value - map to TorBox usenet for backward compatibility
    if (searchSource === 'usenet') {
        return await searchUsenetTracks(query, limit, context);
    }

    // 'both' - search all sources and combine
    if (searchSource === 'both') {
        try {
            ...
            // Sort by cached status and seeders
            tracks.sort((a, b) => {
...
async function searchTorrentTracks(query, limit, context) {
    const sanitized = sanitizeQuery(query);
    const searchLimit = Math.min(limit || CONFIG.MAX_SEARCH_LIMIT, CONFIG.MAX_SEARCH_LIMIT);

    let url = `${MUSIC_API_BASE}/search?q=${encodeURIComponent(sanitized)}&limit=${searchLimit}`;

    const preferredFormat = context?.settings?.preferredFormat?.value;
    if (preferredFormat) {
    ...
    try {
        ...
        const tracks = results.map((item, index) => {
            const infoHash = item.infoHash || item.info_hash || item.InfoHash ||
                           (item.magnetLink || item.magnet_link || item.magnet || '')
                           .match(/xt=urn:btih:([a-fA-F0-9]{40}|[a-zA-Z2-7]{32})/i)?.[1];

            const magnet = item.magnetLink || item.magnet_link ||
                         item.magnet || item.MagnetLink;

            const uniqueId = infoHash
                ? 'tor:' + infoHash
                : 'idx:' + index + ':' + (item.title || '')
                    .substring(0, 20)
                    .replace(/[^a-zA-Z0-9]/g, '');
        ...
        // Auto-check cache status
        let cacheCheckFailed = false;
        try {
            const hashesToCheck = tracks
                .filter(t => t.hash && validateHash(t.hash))
                .map(t => t.hash);
        ...
        // Add warning if cache check failed
        if (cacheCheckFailed) {
...
async function searchUsenetTracks(query, limit, context) {
    const sanitized = sanitizeQuery(query);
    const searchLimit = Math.min(limit || CONFIG.MAX_SEARCH_LIMIT, CONFIG.MAX_SEARCH_LIMIT);
    ...
    try {
        ...
        const tracks = limitedNzbs.map((item, index) => {
            const hash = item.hash || '';
            const uniqueId = hash
                ? 'nzb:' + hash
                : 'nzb:' + index + ':' + (item.title || item.raw_title || '')
                    .substring(0, 20)
                    .replace(/[^a-zA-Z0-9]/g, '');
            ...
            return {
                ...
                year: item.title_parsed_data?.year || null,
                size: item.size || 0,
                sizeFormatted: item.size ? formatBytes(item.size) : 'Unknown',
                nzb: item.nzb,
                hash: hash,
                seeders: -1, // Usenet doesn't have seeders
                leechers: -1, // Usenet doesn't have leechers
                source: item.tracker || 'usenet',
                description: `Usenet - ${item.tracker || 'Unknown Indexer'}`,
        ...
        // Auto-check usenet cache status
        let cacheCheckFailed = false;
        try {
            const hashesToCheck = tracks
                .filter(t => t.hash)
...
function getProwlarrConfig(context) {
    const url = context?.settings?.prowlarrUrl?.value;
    const apiKey = context?.settings?.prowlarrApiKey?.value;
...
// ============================================================================
// NZB PARSING FUNCTIONS
// ============================================================================

function extractFilenameFromSubject(subject) {
    if (!subject) return null;

    // Remove segment counter: (1/N) or [1/N]
    ...
    // Extract filename - look for text with audio extension
    const audioMatch = cleaned.match(/([^\s"]+\.(mp3|flac|wav|aac|ogg|wma|opus|ape|alac))/i);
    if (audioMatch) {
        return audioMatch[1];
    }

    // Fallback: return cleaned subject
    return cleaned;
}

async function parseNzbFile(nzbUrl, prowlarrConfig) {
    // Returns: { success: boolean, filenames: string[], error?: string }
    ...
    try {
        ...
        // Extract filenames from <file subject="..."> attributes
        ...
        console.warn('[MusicTorrent] NZB parse error:', e.message);
        return { success: false, filenames: [], error: e.message };
    }
}

async function expandNzbIntoTracks(nzbRelease, prowlarrConfig) {
    // Takes a single Prowlarr result and expands it into multiple track objects
    // Returns: Array of track objects (one per audio file in NZB)

    try {
        const nzbData = await parseNzbFile(nzbRelease.nzb, prowlarrConfig);

        if (nzbData.success && nzbData.filenames.length > 0) {
            // Create one track object per audio file
            const tracks = nzbData.filenames.map((filename, index) => {
                ...
                return {
                    ...
                    title: parsed.title || filename.replace(/\.[^.]+$/, ''), // Remove extension
                    ...
                    size: nzbRelease.size, // Full NZB size (all files download together)
                    sizeFormatted: nzbRelease.sizeFormatted,
                    nzb: nzbRelease.nzb,  // Same NZB URL for all tracks
                    hash: nzbRelease.hash,
                    seeders: -1,
                    leechers: -1,
                    source: nzbRelease.source,
                    description: nzbRelease.description,
                    ...
                    nzbFilename: filename,  // Individual file this track represents
                    nzbTotalTracks: nzbData.filenames.length,  // Total tracks in NZB
                    nzbParsed: true,
                    index: index  // File index within the NZB
                };
            });
            ...
            // NZB parse failed, return original track as single-item array
        ...
        console.warn('[MusicTorrent] NZB expansion failed for:', nzbRelease.title, e.message);
        return [{
            ...nzbRelease,
            nzbParsed: false,
            nzbParseError: e.message
        }];
    }
}

async function expandNzbsInBatches(nzbReleases, prowlarrConfig, batchSize = 5) {
    // Takes array of NZB releases, expands each into tracks, returns flattened array
    const allTracks = [];

    // Process in batches to avoid overwhelming Prowlarr
...
async function searchProwlarrUsenet(query, limit, context) {
    const sanitized = sanitizeQuery(query);
    const searchLimit = Math.min(limit || CONFIG.MAX_SEARCH_LIMIT, CONFIG.MAX_SEARCH_LIMIT);

    const prowlarr = getProwlarrConfig(context);
    if (!prowlarr) {
        throw new Error('Prowlarr not configured. Please set Prowlarr URL and API key in module settings.');
    }
    ...
    try {
        ...
        // Query params: query, type (optional), categories (optional), limit, offset
        // Category 3000 = Audio, 3010 = Audio/MP3, 3040 = Audio/Lossless, 3050 = Audio/Other

        // Build URL with array parameters for categories
        ...
        // Add categories as separate parameters (Prowlarr expects categories=3000&categories=3010...)
        ...
        // Use all results - we're already filtering by usenet categories in the query
        const usenetResults = results;

        const tracks = usenetResults.map((item, index) => {
            const hash = item.releaseHash || item.infoHash || item.guid || '';
            const uniqueId = hash
                ? 'prowlarr-nzb:' + hash
                : 'prowlarr-nzb:' + index + ':' + (item.title || '')
                    .substring(0, 20)
                    .replace(/[^a-zA-Z0-9]/g, '');
            ...
            return {
                ...
                size: item.size || 0,
                sizeFormatted: item.size ? formatBytes(item.size) : 'Unknown',
                nzb: item.downloadUrl, // NZB download URL
                hash: hash,
                seeders: -1, // Usenet doesn't have seeders
                leechers: -1,
                source: item.indexer || 'prowlarr',
                description: `Prowlarr - ${item.indexer || 'Unknown Indexer'}`,
                cached: false, // Will check cache separately
                type: 'usenet',
                prowlarrGuid: item.guid, // Keep for reference
                publishDate: item.publishDate,
                categories: item.categories || [],
                index: 0  // Unparsed NZBs default to index 0
            };
        });

        // ===== NZB EXPANSION SECTION =====
        // Expand each NZB release into individual track objects
        ...
        try {
            // Rename 'tracks' to 'nzbReleases' for clarity
            const nzbReleases = tracks;
            expandedTracks = await expandNzbsInBatches(nzbReleases, prowlarr, 5);

            // Calculate stats
            const parsedTracksCount = expandedTracks.filter(t => t.nzbParsed === true).length;
            const failedTracksCount = expandedTracks.filter(t => t.nzbParsed === false).length;

            // Count unique NZBs that were successfully parsed
            const uniqueNzbs = new Set(expandedTracks.map(t => t.nzb));
            const parsedNzbs = new Set(expandedTracks.filter(t => t.nzbParsed === true).map(t => t.nzb));
            ...
            console.log(`[MusicTorrent] Expanded ${nzbReleases.length} NZBs into ${expandedTracks.length} tracks`);
        ...
        // Check cache status if TorBox is configured
        let cacheCheckFailed = false;
        try {
            const hashesToCheck = expandedTracks
                .filter(t => t.hash)
                .map(t => t.hash);
...
async function fetchWithRetry(url, options, context = 'API call') {
    let attempt = 0;

    while (attempt < CONFIG.RATE_LIMIT_MAX_RETRIES) {
        ...
        if (attempt >= CONFIG.RATE_LIMIT_MAX_RETRIES) {
            throw new Error(`Rate limit exceeded after ${CONFIG.RATE_LIMIT_MAX_RETRIES} attempts`);
        }

        // Exponential backoff: 1s, 2s, 4s, 8s, 16s (capped at 30s)
        const waitTime = Math.min(
            CONFIG.RATE_LIMIT_BASE_DELAY * Math.pow(2, attempt - 1),
            CONFIG.RATE_LIMIT_MAX_DELAY
        );

        console.log(`[MusicTorrent] Rate limited (429) for ${context}, waiting ${waitTime}ms... (Attempt ${attempt}/${CONFIG.RATE_LIMIT_MAX_RETRIES})`);
...
async function resolveViaAPI(infoHash, service, apiKey, fileIndex = 0) {
...
async function processTorBox(magnet, context, trackId) {
    ...
    if (!addResp || !addResp.success) {
        if (addResp?.detail?.includes('already exists')) {
            ...
            const listResp = await fetch(
                TORBOX_API_BASE + '/torrents/mylist?bypass_cache=true',
...
async function processTorBoxTorrent(torrentId, context, trackId) {
    let fileId = null;
    let attempts = 0;
    ...
    while (attempts < CONFIG.TORRENT_POLL_ATTEMPTS) {
        ...
        if (info.success && info.data) {
            ...
            if (data) {
                const isReady = data.download_state === 'cached' ||
    ...
    if (!fileId) {
        throw new Error(
            'Torrent processing timed out or no audio files found. ' +
            'Check your TorBox dashboard.'
        );
    }
...
async function processUsenetDownload(nzbUrl, context, trackId) {
    console.log('[MusicTorrent] Adding NZB to TorBox usenet');

    const formData = new FormData();
    formData.append('link', nzbUrl);
    formData.append('seed', '1');
    formData.append('post_processing', '-1'); // Default post-processing
    ...
    if (!addResp || !addResp.success) {
        if (addResp?.detail?.includes('already exists')) {
            ...
            const listResp = await fetch(
                TORBOX_API_BASE + '/usenet/mylist?bypass_cache=true',
                { headers: { 'Authorization': 'Bearer ' + context.debridApiKey } }
            ).then(r => r.json());
            ...
            // Extract hash from trackId (format: "nzb:hash" or "nzb:index:partial")
            const hash = trackId.startsWith('nzb:') ? trackId.substring(4).split(':')[0] : trackId;

            // Try to find by hash first, then by exact name match
...
async function processUsenetItem(usenetId, context, trackId, trackIndex = 0) {
    let fileId = null;
    let attempts = 0;
    ...
    while (attempts < CONFIG.TORRENT_POLL_ATTEMPTS) {
        ...
        const info = await fetch(
            TORBOX_API_BASE + '/usenet/mylist?id=' + usenetId,
            { headers: { 'Authorization': 'Bearer ' + context.debridApiKey } }
        ).then(r => r.json());
            ...
            if (data) {
                ...
                if (isReady && data.files && data.files.length > 0) {
                    ...
                    if (media.length > 0) {
                        ...
                        // Use trackIndex to select the specific file (for expanded NZB tracks)
                        if (trackIndex < media.length) {
                            fileId = media[trackIndex].id;
                        } else {
                            // Fallback: use first file if index out of bounds
                            fileId = media[0].id;
                        }
                        break;
                    }
    ...
    if (!fileId) {
        throw new Error(
            'Usenet processing timed out or no audio files found. ' +
            'Check your TorBox dashboard.'
        );
    }
...
async function processRealDebrid(magnet, context, trackId) {
    ...
    throw new Error(
        'Torrent processing timed out. Check your Real-Debrid dashboard for status.'
    );
}
...
async function getUsenetStreamUrl(trackId, quality, context) {
    // Handle both regular nzb: and prowlarr-nzb: prefixes
    ...
    if (context.debridProvider !== 'torbox') {
        throw new Error(ERRORS.UNSUPPORTED_PROVIDER(context.debridProvider));
    }

    // Extract hash and track index from nzb trackId
    // Format: "prowlarr-nzb:hash123-track-0" or "nzb:hash123"
    let hash;
    let trackIndex = 0; // Default to first file

    if (trackId.startsWith('prowlarr-nzb:')) {
        const afterPrefix = trackId.substring(13); // Remove "prowlarr-nzb:" prefix
        // Check for "-track-N" suffix (from expanded NZBs)
        ...
        hash = trackId.substring(4); // Remove "nzb:" prefix
    }

    // First, try to find the usenet download in the user's list
    const listResp = await fetch(
        TORBOX_API_BASE + '/usenet/mylist',
        { headers: { 'Authorization': 'Bearer ' + context.debridApiKey } }
    ).then(r => r.json());
    ...
    let usenetDownload = downloads.find(d =>
    ...
    // If not found, we need to create the download from the original NZB
    if (!usenetDownload) {
        // This would require access to the original NZB URL, which we might not have
        // For now, throw an error suggesting the user add it to cloud first
...
async function getTrackStreamUrl(trackId, quality, context) {
    // Handle usenet tracks (both TorBox Search and Prowlarr)
    ...
    const provider = mapProviderName(context.debridProvider);
    ...
    try {
        ...
        console.warn('[MusicTorrent] API resolve failed, using fallback:', e.message);
    }
    ...
    if (context.debridProvider === 'realdebrid') {
        ...
        throw new Error(
            `Direct processing not implemented for ${context.debridProvider}. ` +
...
async function addToCloud(track, context) {
    // Handle usenet tracks from any source
    if (track.id.startsWith('nzb:') ||
    ...
    if (provider === 'torbox') {
        ...
        const selectResp = await fetch(
            REALDEBRID_API_BASE + '/torrents/selectFiles/' + addData.id,
            {
                method: 'POST',
                headers: {
                    'Authorization': 'Bearer ' + apiKey,
...
async function addUsenetToCloud(track, context) {
    const debrid = getEffectiveDebridKey(context);
...
async function getCloudAlbums(context) {
    const debrid = getEffectiveDebridKey(context);
    ...
    try {
        ...
        // Fetch both torrents and usenet downloads in parallel
        ...
        if (torrentResponse.status === 'fulfilled' && torrentResponse.value.ok) {
            const torrentData = await torrentResponse.value.json();
            if (torrentData.success) {
                ...
                const torrentAlbums = musicTorrents.map(t => ({
                    id: 'torrent:' + t.id,
                    hash: t.hash,
                    name: t.name,
                    size: t.size,
        ...
        if (usenetResponse.status === 'fulfilled' && usenetResponse.value.ok) {
            const usenetData = await usenetResponse.value.json();
            if (usenetData.success) {
                ...
                const usenetAlbums = musicUsenet.map(u => ({
                    id: 'usenet:' + u.id,
                    hash: u.hash,
                    name: u.name,
                    size: u.size,
...
async function getTorrentFiles(torrentId, context) {
    // Handle usenet downloads
    if (torrentId.startsWith('usenet:')) {
        return await getUsenetFiles(torrentId.substring(7), context);
    }

    // Remove torrent: prefix if present
    ...
    try {
        const response = await fetch(
            TORBOX_API_BASE + '/torrents/mylist?id=' + cleanId,
            {
                method: 'GET',
                headers: {
                    'Authorization': 'Bearer ' + debrid.apiKey
...
async function getUsenetFiles(usenetId, context) {
...
async function getUsenetFileStreamUrl(usenetId, fileId, context) {
    const debrid = getEffectiveDebridKey(context);
    ...
    try {
        ...
        console.log('[MusicTorrent] Requesting usenet download URL with params:', { usenet_id: usenetId, file_id: fileId });
        ...
        if (!response.ok) {
            ...
            console.error('[MusicTorrent] Initial request failed with status', response.status, ':', errorBody);

            // Try requesting zip of entire download as fallback
            ...
            if (!fallbackResp.ok) {
                ...
                console.error('[MusicTorrent] Fallback request failed with status', fallbackResp.status, ':', fallbackError);
                throw new Error('HTTP ' + fallbackResp.status + ' - Fallback request (zip) failed to get usenet stream URL');
            }

            const fallbackData = await fallbackResp.json();
            return fallbackData.success ? fallbackData.data : null;
        }
...
async function getStreamUrl(torrentId, fileId, context) {
    // Handle usenet downloads
    if (torrentId.startsWith('usenet:')) {
    ...
    try {
        ...
        if (!response.ok) {
            const errorBody = await response.text().catch(() => '');
            console.error('[MusicTorrent] Initial torrent request failed with status', response.status, ':', errorBody);

            // Try without file_id for single-file torrents
            const fallbackParams = new URLSearchParams({
                token: debrid.apiKey,
                torrent_id: cleanId,
                zip_link: 'true'
            ...
            if (!fallbackResp.ok) {
                const fallbackError = await fallbackResp.text().catch(() => '');
                console.error('[MusicTorrent] Fallback torrent request failed with status', fallbackResp.status, ':', fallbackError);
                throw new Error('HTTP ' + fallbackResp.status + ' - Fallback request (zip) failed to get stream URL');
            }
